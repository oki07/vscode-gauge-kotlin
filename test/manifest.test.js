...
// The real TextMate engine executing getgauge/gauge-vscode
// syntaxes/markdown.tmLanguage releases persistent embedded states through
// an inner while rule at the closing fence. Language scopes belong to that body.
test("Gauge TextMate grammar preserves common Markdown constructs", () => {
  ...
  assert.equal(markdownCssFence.patterns[0].contentName, "meta.embedded.block.css");
  assert.deepEqual(markdownCssFence.patterns[0].patterns, [{ include: "source.css" }]);
  assertPatternMatches(markdownBasicFence, "```html", "```html");
  assertPatternMatches(markdownBasicFence, "~~~xhtml", "~~~xhtml");
  assert.equal(markdownBasicFence.patterns[0].contentName, "meta.embedded.block.html");
  assert.deepEqual(markdownBasicFence.patterns[0].patterns, [{ include: "text.html.basic" }]);
  assertPatternMatches(markdownJavaFence, "```java", "```java");
  assertPatternMatches(markdownJavaFence, "~~~bsh", "~~~bsh");
  assert.equal(markdownJavaFence.patterns[0].contentName, "meta.embedded.block.java");
  assert.deepEqual(markdownJavaFence.patterns[0].patterns, [{ include: "source.java" }]);
  ...
  assert.equal(markdownKotlinFence.patterns[0].contentName, "meta.embedded.block.kotlin");
  assert.deepEqual(markdownKotlinFence.patterns[0].patterns, [{ include: "source.kotlin" }]);
  ...
  assert.equal(markdownJavaScriptFence.patterns[0].contentName, "meta.embedded.block.javascript");
  assert.deepEqual(markdownJavaScriptFence.patterns[0].patterns, [{ include: "source.js" }]);
  assertPatternMatches(markdownJsonFence, "```json", "```json");
  assertPatternMatches(markdownJsonFence, "~~~sublime-settings", "~~~sublime-settings");
  assert.equal(markdownJsonFence.patterns[0].contentName, "meta.embedded.block.json");
  ...
  assert.equal(markdownPythonFence.patterns[0].contentName, "meta.embedded.block.python");
  assert.deepEqual(markdownPythonFence.patterns[0].patterns, [{ include: "source.python" }]);
  assertPatternMatches(markdownSqlFence, "```ddl", "```ddl");
  assert.equal(markdownSqlFence.patterns[0].contentName, "meta.embedded.block.sql");
  assert.deepEqual(markdownSqlFence.patterns[0].patterns, [{ include: "source.sql" }]);
  assertPatternMatches(markdownXmlFence, "```xml", "```xml");
  assertPatternMatches(markdownXmlFence, "~~~cpt", "~~~cpt");
  assert.equal(markdownXmlFence.patterns[0].contentName, "meta.embedded.block.xml");
  assert.deepEqual(markdownXmlFence.patterns[0].patterns, [{ include: "text.xml" }]);
  assertPatternMatches(markdownDockerfileFence, "```Dockerfile", "```Dockerfile");
  assert.equal(markdownDockerfileFence.patterns[0].contentName, "meta.embedded.block.dockerfile");
  assert.deepEqual(markdownDockerfileFence.patterns[0].patterns, [{ include: "source.dockerfile" }]);
  assertPatternMatches(markdownGoFence, "```golang", "```golang");
  assert.equal(markdownGoFence.patterns[0].contentName, "meta.embedded.block.go");
  assert.deepEqual(markdownGoFence.patterns[0].patterns, [{ include: "source.go" }]);
  assertPatternMatches(markdownRustFence, "```rs", "```rs");
  assert.equal(markdownRustFence.patterns[0].contentName, "meta.embedded.block.rust");
  assert.deepEqual(markdownRustFence.patterns[0].patterns, [{ include: "source.rust" }]);
  ...
  assert.equal(markdownShellFence.patterns[0].contentName, "meta.embedded.block.shellscript");
  assert.deepEqual(markdownShellFence.patterns[0].patterns, [{ include: "source.shell" }]);
  assertPatternMatches(markdownTypeScriptFence, "```typescript", "```typescript");
  assertPatternMatches(markdownTypeScriptFence, "~~~ts", "~~~ts");
  assert.equal(markdownTypeScriptFence.patterns[0].contentName, "meta.embedded.block.typescript");
  assert.deepEqual(markdownTypeScriptFence.patterns[0].patterns, [{ include: "source.ts" }]);
  assertPatternMatches(markdownTsxFence, "```tsx", "```tsx");
  assert.equal(markdownTsxFence.patterns[0].contentName, "meta.embedded.block.typescriptreact");
  assert.deepEqual(markdownTsxFence.patterns[0].patterns, [{ include: "source.tsx" }]);
  assertPatternMatches(markdownCSharpFence, "```c#", "```c#");
  assert.equal(markdownCSharpFence.patterns[0].contentName, "meta.embedded.block.csharp");
  assert.deepEqual(markdownCSharpFence.patterns[0].patterns, [{ include: "source.cs" }]);
  assertPatternMatches(markdownYamlFence, "```yaml", "```yaml");
  assertPatternMatches(markdownYamlFence, "~~~yml", "~~~yml");
  assert.equal(markdownYamlFence.patterns[0].contentName, "meta.embedded.block.yaml");
  assert.deepEqual(markdownYamlFence.patterns[0].patterns, [{ include: "source.yaml" }]);
