...
// A data-table row id is "<spec>:<line>_<row>" (src/execution/lineProcessors.js
...
test("GaugeTestController parents a table row under its own scenario", () => {
  ...
  gaugeTests.currentRequest = {
    include: [
      { id: "/w/specs/a.spec:10" },
      { id: "/w/specs/a.spec:20" },
    ],
  };
  ...
  assert.equal(
    gaugeTests.parentIdForEvent({
      type: "testFailed",
      id: "/w/specs/a.spec:20_2",
      parentId: "/w/specs/a.spec",
      resultOnly: true,
    }),
    "/w/specs/a.spec:20",
  );
